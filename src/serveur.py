...
load_dotenv()
WD = os.getenv('WD')
for i in os.listdir(WD+'src/WEB/temp'):
...
def put_data(key,value):
    ...
    try :
        with open(WD + 'src/WEB/memory/last_cache.json','w') as f : f.write(json.dumps(cache.cache_data))
    except Exception as e:
        print(f'Error : {e}')
    return jsonify({
        'status':'Logged',
...
def launch_backtest():
    try:
        ...
        thread = threading.Thread(target=launching_backtest,args=(data,))
        thread.start()
...
def make_full_graph():
    ...
    return jsonify(get_data_graph(f'{cache.cache_data["datafile"]}.csv',f'{WD}src/OPE/reporting/data_logger/Order.csv',f'{WD}src/OPE/reporting/data_logger/Position.csv'))
